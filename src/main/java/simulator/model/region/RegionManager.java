...
public class RegionManager implements AnimalMapView {
	...
	public RegionManager(int cols, int rows, int width, int height) {
		if (cols <= 0)
			throw new IllegalArgumentException("RegionManager: constructora -> cols: debe ser positiva");
		if (rows <= 0)
			throw new IllegalArgumentException("RegionManager: constructora -> rows: debe ser positiva");
		if (width <= 0)
			throw new IllegalArgumentException("RegionManager: constructora -> width: debe ser positiva");
		if (height <= 0)
			throw new IllegalArgumentException("RegionManager: constructora -> height: debe ser positiva");
		if (width % cols != 0)
			throw new IllegalArgumentException("RegionManager: width debe ser divisible entre cols");
		if (height % rows != 0)
			throw new IllegalArgumentException("RegionManager: height debe ser divisible entre rows");
		this.cols = cols;
		this.rows = rows;
		this.mapWidth = width;
	...
	public Iterator<MapInfo.RegionData> iterator() {
		return new Iterator<MapInfo.RegionData>() {
			...
			public MapInfo.RegionData next() {
				...
				MapInfo.RegionData data = new MapInfo.RegionData(currentRow, currentCol,
						regions[currentRow][currentCol]);
